...
/**
 * This is the model class for table "t_fileinfo".
 *
 * The followings are the available columns in table 't_fileinfo':
...
class Fileinfo extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('IsCover', 'numerical', 'integerOnly'=>true),
			array('fileID, yaoheID', 'length', 'max'=>36),
			array('filename, filepath', 'length', 'max'=>255),
			array('uploadtime', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('fileID, filename, filepath, uploadtime, IsCover, yaoheID', 'safe', 'on'=>'search'),
		);
	}
